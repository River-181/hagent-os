...
async function recordDeliveryArtifacts(
  ...
  input: {
    ...
    delivery: Awaited<ReturnType<typeof sendKakaoMessage>>
    actor: "system" | "user"
  },
) {
  const action =
  ...
  await db.insert(schema.activityEvents).values({
    ...
    entityTitle: `${input.caseRecord.identifier} kakao reply ${input.delivery.status}`,
    metadata: {
...
export async function processKakaoApprovalDelivery(
  db: Db,
  approvalId: string,
  options?: {
    ...
    await db.insert(schema.notifications).values({
      ...
      title:
        delivery.status === "failed"
          ? `${caseRecord.identifier} 카카오 발송 실패`
          : `${caseRecord.identifier} 카카오 발송 준비`,
      body:
        delivery.status === "failed"
