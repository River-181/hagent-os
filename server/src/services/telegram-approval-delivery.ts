...
async function recordDeliveryArtifacts(
  ...
  await upsertOutboundDeliveryDocument(db, {
    ...
    body: [
      ...
      .filter(Boolean)
      .join("\n"),
    artifactTag: "artifact:telegram-reply",
    status: input.delivery.status,
  })
}
...
export async function processTelegramApprovalDelivery(
  ...
  const draft = buildOutboundReplyDraft("telegram", approval)
  if (!draft) throw new Error("No Telegram reply draft available")

  const existingDelivery = getExistingOutboundDelivery("telegram", approval)
  ...
  // 실시간 오케스트레이터 메모리 갱신 (best-effort, 비동기)
